...
/datum/component/masquerade_hud/proc/create_masquerade_overlay()
	...
	if(!atom_parent.hud_list[HUD_LIST_MASQUERADE])
		atom_parent.hud_list[HUD_LIST_MASQUERADE] = image(icon = 'icons/obj/storage/closet.dmi', loc = atom_parent, icon_state = "cardboard_special", layer = ABOVE_ALL_MOB_LAYER, pixel_z = 32)
