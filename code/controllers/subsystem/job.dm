SUBSYSTEM_DEF(job)
	...
	var/overflow_role = /datum/job/vampire/citizen //DARKPACK EDIT, ORIGINAL: var/overflow_role = /datum/job/assistant
...
/datum/controller/subsystem/job/proc/setup_occupations()
	...
	var/list/all_jobs = subtypesof(/datum/job/vampire) //DARKPACK EDIT, ORIGINAL: var/list/all_jobs = subtypesof(/datum/job)
	if(!length(all_jobs))
